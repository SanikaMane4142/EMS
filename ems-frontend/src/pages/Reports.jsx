...
const Reports = () => {
  ...
  const handleReview = async (rep) => {
    try {
      ...
      Swal.fire({ icon: 'success', title: 'Reviewed', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
    } catch (err) {
...
Generated by EMS
  ...
  const handleExportAll = () => {
    if (filteredReports.length === 0) {
      Swal.fire({ icon: 'info', title: 'No Data', text: 'No reports to export matching current filters.' });
      return;
    }
      ...
      >
        {selectedReport && (
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, minHeight: 500 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, position: 'relative', zIndex: 1 }}>
                ...
                <Box sx={{ p: 2.5, bgcolor: '#ffffff', borderRadius: '20px', border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography sx={{ fontSize: '1.125rem', fontWeight: 900, color: '#0f172a' }}>{selectedReport.productivity_rating} / 10</Typography>
                    <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5 }}>
                      {[1, 2, 3, 4, 5].map(star => (
                        <Star
                          key={star} size={12}
                          fill={star <= Math.round(selectedReport.productivity_rating / 2) ? "#f59e0b" : "none"}
                          color={star <= Math.round(selectedReport.productivity_rating / 2) ? "#f59e0b" : "#e2e8f0"}
                        />
                      ))}
                    </Box>
                  </Box>
                ...
                <Box sx={{ p: 2.5, bgcolor: '#ffffff', borderRadius: '20px', border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography sx={{ fontSize: '1.125rem', fontWeight: 900, color: '#0f172a' }}>{selectedReport.total_working_hours} hrs</Typography>
                  </Box>
                </Box>
              </Box>
            ...
            {/* RIGHT CONTENT */}
            <Box sx={{ flex: 1, bgcolor: '#ffffff', p: 5, display: 'flex', flexDirection: 'column' }}>
                ...
                {[
                  ...
                  <Box key={idx} sx={{
                    ...
                    <Box sx={{
                      ...
                      <Typography sx={{ fontSize: '0.75rem', fontWeight: 500, color: '#94a3b8', maxWidth: 200, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {item.value}
                      </Typography>
                    </Box>
                  </Box>
